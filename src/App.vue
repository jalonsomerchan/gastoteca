  ...
  <div class="app-shell" @click="dismissHeaderMenu" @pointerdown.capture="dismissSmartSelectOutside">
    ...
    <div v-if="modalOpen" class="modal-backdrop" @mousedown.self="closeExpenseModal">
      <section class="modal" role="dialog" aria-modal="true" aria-labelledby="expense-title">
        <header><div><p class="eyebrow">{{ quickExpenseMode ? 'GASTO RÁPIDO' : draft.id ? 'EDITAR MOVIMIENTO' : 'NUEVO MOVIMIENTO' }}</p><h2 id="expense-title">{{ quickExpenseMode ? 'Guardar importe' : draft.id ? 'Editar movimiento' : draft.transaction_type ? (draft.transaction_type === 'income' ? 'Nuevo ingreso' : 'Nuevo gasto') : 'Nuevo movimiento' }}</h2></div><button class="icon-button" aria-label="Cerrar" @click="closeExpenseModal"><PhX :size="22" /></button></header>
        ...
        <form v-else @submit.prevent="saveExpense">
          ...
          <div class="form-grid">
            ...
            <label><span>Fecha y hora (opcional)</span><input v-model="draft.occurred_at" type="datetime-local" /></label>
