...
<template>
  ...
  >
    ...
    <div class="expense-main">
      <strong>{{ expense.name }} <em v-if="expense.transaction_type === 'income'" class="movement-type">Ingreso</em><em v-if="expense.is_quick" class="quick-pending-pill">Por completar</em></strong><span><PhMapPin aria-hidden="true" :size="14" /> {{ expense.place || 'Sin establecimiento' }} · {{ dateLabel(expense.occurred_at) }}</span><small v-if="expense.details" class="expense-details">{{ expense.details }}</small><span v-if="expense.tags?.length" class="expense-tag-list"><em v-for="tag in expense.tags" :key="tag">{{ tag }}</em></span>
    </div>
